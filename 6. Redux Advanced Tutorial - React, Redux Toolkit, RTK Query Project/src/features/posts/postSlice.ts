  ...
  endpoints: (builder) => ({
    getPosts: builder.query({
      ...
      transformResponse: (responseData: unknown) => {
        let min = 1;
        const loadedPosts = (responseData as Post[]).map((post: Post) => {
          if (!post?.date)
...
export const selectPostsResult = extendedApiSlice.endpoints.getPosts.select(
  (state: any) => state // TODO: observe if this is correct
);
...
const selectPostsData = createSelector(
  ...
  selectPostsResult,
  // second argument is a function that receives the input selectors as arguments
